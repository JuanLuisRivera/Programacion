...
int main(int argc, char** argv)
{
	...
	while (input[0] != 'q')
	{
		...
		// comments as needed
        inputValues.theta = inputValues.theta * (M_PI / 180);

        float v0_x = inputValues.speed * cosf(inputValues.theta);

		float v0_y = inputValues.speed * sinf(inputValues.theta);

		float t = v0_y / G;
		
		float h = powf (v0_y, 2) / (2 * G);
